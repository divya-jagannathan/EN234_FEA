...
!
       ...
       double precision E,xnu,Y,e0,m,n,edot0,S,H,Omega,alpha
       double precision f,dfde
       double precision eplas,deplas,ta,dta,val,ddtade,dcde
!
...
!     Elastic increment (either no stress, or no plastic strain rate)
       ...
       do while (err>tol)
            ...
            if (val<1.d-08) then
            dta=1-ta*val**2
            ddtade = -2*ta*val/Omega
            else
            dta=(dt/val-ta*(1-exp(-val)))/(1+(1/val)*exp(-val))
            ddtade=(-Omega*dt/(deplas)**2 + Omega*(dta/(deplas)**2)*
     1        exp(-val)-(ta-dta/val)*exp(-val)/Omega)/(1+(1/val)*
     2        exp(-val))
            endif
           
            c1 = (Y/S)*(1+(eplas+deplas)/e0)**m +
     1      H*(1-exp(-((ta+dta)/td)**alpha)) +log(deplas/(dt*edot0))

             f = sestar/S - 1.5d0*E*deplas/(S*(1.d0+xnu))-c1
             
             dcde=exp(-((ta+dta)/td)**alpha)*alpha*((ta+dta)/td)**(alpha
     1        -1)*(1/td)*ddtade
             
             dfde =  - 1.5d0*E/(1.d0+xnu) -((Y*m)/(S*e0))*
     1        (1+(eplas+deplas)/e0)**(m-1) - 1/deplas -H*dcde  
